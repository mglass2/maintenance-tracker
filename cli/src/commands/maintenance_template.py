    ...
    while True:
        ...
        try:
            with APIClient() as client:
                ...
                # Filter out task types that already have maintenance templates
                existing_template_task_ids = {
                    template.get("task_type_id")
                    for template in existing_templates
                }
                available_task_types = [
                    task_type for task_type in task_types
                    if task_type.get("id") not in existing_template_task_ids
                ]
        ...
        try:
            with APIClient() as client:
                ...
                if response.status_code == 201:
                    ...
                    while True:
                        create_another = click.prompt(
                            f"\nWould you like to create another template for {selected_item_type_name}? (yes/no)",
                            type=str,
                            default="no"
                        ).strip().lower()
